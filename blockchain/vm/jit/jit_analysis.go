...
func (az *traceAnalyzer) finishSegment() {
	{
		for i := az.segOffset; i < az.segOffset+az.segLen; i++ {
			fmt.Println("!!!", hexutil.Encode([]byte{az.code[i]}))
		}
	}
	...
	for az.pc < uint64(len(az.code)) {
		...
		if az.scanningForJumpdest {
			if op == 0x5b { // JUMPDEST 발견!
				...
				// 여기서부터 새로운 블록 분석 시작
				// (JUMPDEST는 Supported Opcode이므로 아래 로직에서 처리됨)
		...
		if op == 0x56 {
			...
			// Dynamic Jump -> 흐름 끊김!
			az.discardSegment() // 어디로 갈지 모르니 현재 블록도 위험 -> 버림
